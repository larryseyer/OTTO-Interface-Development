...
class TimerManager {
  ...
  setTimeout(callback, delay, category = "general", name = null) {
    ...
    if (this.debugMode) {
      console.log(
        `TimerManager: Created timeout ${timerInfo.name} (${timerId})`,
      );
    }
  ...
  setInterval(callback, delay, category = "general", name = null) {
    ...
    if (this.debugMode) {
      console.log(
        `TimerManager: Created interval ${timerInfo.name} (${timerId})`,
      );
    }
  ...
  requestAnimationFrame(callback, name = null) {
    ...
    if (this.debugMode) {
      console.log(
        `TimerManager: Created animation frame ${frameInfo.name} (${frameId})`,
      );
    }
  ...
  clearCategory(category) {
    ...
    if (this.debugMode) {
      console.log(
        `TimerManager: Cleared ${cleared} timers in category ${category}`,
      );
    }
  ...
  pauseCategory(category) {
    ...
    this.categories[category].forEach((timerId) => {
      ...
      if (timerInfo && !timerInfo.paused) {
        if (timerInfo.nativeId) {
          if (timerInfo.type === "timeout") {
            clearTimeout(timerInfo.nativeId);
            timerInfo.remainingDelay =
              timerInfo.delay - (Date.now() - timerInfo.createdAt);
          } else if (timerInfo.type === "interval") {
            clearInterval(timerInfo.nativeId);
          }
    ...
    if (this.debugMode) {
      console.log(
        `TimerManager: Paused ${paused} timers in category ${category}`,
      );
    }
  }
  ...
  resumeCategory(category) {
    ...
    if (this.debugMode) {
      console.log(
        `TimerManager: Resumed ${resumed} timers in category ${category}`,
      );
    }
  }
