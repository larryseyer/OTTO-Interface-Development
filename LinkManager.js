...
class LinkManager {
  ...
  initialize() {
    ...
    if (this.playerStateManager) {
      this.playerStateManager.addListener(
        "*",
        (newState, oldState, playerNum) => {
  ...
  handlePlayerStateChange(playerNum, newState, oldState) {
    ...
    if (this.propagating) {
      ...
      const expected = this.propagationStack.find(
        (p) => p.player === playerNum && p.timestamp > Date.now() - 1000,
      );
      if (expected) {
    ...
    if (
      newState &&
      oldState &&
      newState.sliderValues &&
      oldState.sliderValues
    ) {
      this.linkableParameters.forEach((param) => {
  ...
  handleLinkedValueChange(sourcePlayer, parameter, value) {
    ...
    const linkedPlayers = Array.from(group.keys()).filter(
      (p) => p !== sourcePlayer,
    );
  ...
  propagateValue(parameter, value, sourcePlayer, targetPlayers) {
    ...
    const now = Date.now();
    this.propagationStack = this.propagationStack.filter(
  ...
  setMaster(parameter, playerNum) {
    ...
    if (!group || !group.has(playerNum)) {
      throw new Error(
        `Player ${playerNum} is not in link group for ${parameter}`,
      );
    }
    ...
    if (this.playerStateManager) {
      ...
      if (masterState && masterState.sliderValues) {
        ...
        const otherPlayers = Array.from(group.keys()).filter(
          (p) => p !== playerNum,
        );
  ...
  updateSliderUI(playerNum, parameter, value) {
    ...
    if (currentPlayer === playerNum) {
      const slider = document.querySelector(
        `.custom-slider[data-param="${parameter}"]`,
      );
      if (slider) {
  ...
  exportStates() {
    ...
    this.linkGroups.forEach((group, param) => {
      exported.linkGroups[param] = Array.from(group.entries()).map(
        ([player, info]) => ({
          player,
          ...info,
        }),
