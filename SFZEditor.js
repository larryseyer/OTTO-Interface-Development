class SFZEditor {
  ...
  validate() {
    ...
    this.regions.forEach((region, index) => {
      ...
      if (
        region.lovel !== undefined &&
        (region.lovel < 0 || region.lovel > 127)
      ) {
        errors.push(`Region ${index}: Invalid lovel value ${region.lovel}`);
      }
      if (
        region.hivel !== undefined &&
        (region.hivel < 0 || region.hivel > 127)
      ) {
      ...
      if (!region.pitch_keycenter && region.key) {
        warnings.push(
          `Region ${index}: No pitch_keycenter defined, sample may be pitch-shifted`,
        );
      }
    });
