class MidiTranslator {
  ...
  detectVendorFormat(midiEvents) {
    ...
    vendors.forEach((vendor) => {
      ...
      if (vendor.keyNotes.length > 0) {
        ...
        if (
          totalNotes >
          Object.values(noteUsage).reduce((a, b) => a + b, 0) * 0.5
        ) {
  ...
  translateMidiFile(midiData, sourceVendor, targetVendor = "generalMidi") {
    const parsedMidi =
      typeof midiData === "object" ? midiData : this.parseMidiFile(midiData);
    const translatedEvents = [];

    // Use detected vendor if not specified
    if (!sourceVendor && parsedMidi.detectedVendor) {
      sourceVendor =
        parsedMidi.detectedVendor.vendor || parsedMidi.detectedVendor;
    }
  ...
  getTranslationMap(sourceVendor, targetVendor) {
    ...
    for (let note = 0; note < 128; note++) {
      map[note] = this.drumMapPresets.translateNote(
        sourceVendor,
        targetVendor,
        note,
      );
  ...
  translateRealtimeNote(note, velocity, sourceVendor = null) {
    ...
    const targetVendor =
      currentMap.vendor === "OTTO" ? "generalMidi" : currentMap.id;
    const translationMap = this.getTranslationMap(sourceVendor, targetVendor);
  ...
  analyzeMidiFile(midiData) {
    ...
    parsedMidi.events.forEach((event) => {
      if (event.type === "noteOn") {
        // Count notes
        analysis.noteCount[event.note] =
          (analysis.noteCount[event.note] || 0) + 1;
  ...
  convertToPatternGrid(midiEvents, resolution = 16) {
    const grid = {};
    const quantize = (time, resolution) =>
      Math.round(time * resolution) / resolution;
  ...
  getVendorCompatibility(sourceVendor, targetVendor) {
    ...
    Object.keys(sourcePreset.mapping).forEach((note) => {
      total++;
      const translated = this.drumMapPresets.translateNote(
        sourceVendor,
        targetVendor,
        parseInt(note),
      );
    ...
    if (report.coverage < 50) {
      report.recommendations.push(
        "Consider using General MIDI as intermediate format",
      );
    }
    if (report.missingMappings.length > 10) {
      report.recommendations.push(
        "Manual mapping may be required for some sounds",
      );
    }
