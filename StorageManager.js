...
class StorageManager {
    ...
    try {
      ...
      if (error.name === "QuotaExceededError") {
        ...
        try {
          ...
          console.error(
            "StorageManager: Save failed after cleanup",
            retryError,
          );
          return false;
        }
  ...
  async handleQuotaExceeded() {
    ...
    for (const item of storageInfo.items) {
      try {
        ...
        const data = JSON.parse(
          value.startsWith("LZ:") ? await this.decompress(value) : value,
        );
    ...
    if (itemsToDelete.length === 0) {
      const nonEssential = storageInfo.items
        .filter(
          (item) =>
            !item.key.includes("preset") && !item.key.includes("default"),
        )
        .slice(0, 5);
  ...
  cleanupOldData() {
    ...
    if (keysToDelete.length > 0) {
      console.log(
        `StorageManager: Cleaned up ${keysToDelete.length} old items`,
      );
    }
  }
  ...
  async recoverCorruptedData(key) {
    console.warn(
      `StorageManager: Attempting to recover corrupted data for ${key}`,
    );
  ...
  async runMigrations() {
    ...
    console.log(
      `StorageManager: Migrating from ${currentVersion} to ${this.version}`,
    );
  ...
  async migrateData(key, wrappedData) {
    ...
    if (migration) {
      try {
        ...
        console.error(
          `StorageManager: Data migration failed for ${key}`,
          error,
        );
      }
    }
