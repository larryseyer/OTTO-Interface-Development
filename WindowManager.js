...
class WindowManager {
    ...
    initializeRegistry() {
        ...
        this.registerWindow('panel', 'store', {
            ...
            onOpen: () => {
                ...
                if (iframe && !iframe.hasAttribute('data-loaded')) {
                    ...
                    const storeURL = this.otto.storeURL || 'https://my-store-1008202.creator-spring.com/';
        ...
        this.registerWindow('panel', 'mixer', {
            ...
            onOpen: () => {
                ...
                const selectedKit = document.querySelector('#kit-dropdown .dropdown-text');
    ...
    async closeAllWindows(exceptType = null, exceptName = null) {
        ...
        for (const [type, windows] of Object.entries(this.states)) {
            for (const [name, state] of Object.entries(windows)) {
                if (state.isOpen) {
                    const skipThis = exceptType && exceptName && 
                                    type === exceptType + 's' && name === exceptName;
                    if (!skipThis) {
    ...
    isWindowOpen(type, name) {
        ...
        return this.states[stateKey] && this.states[stateKey][name] && 
    ...
    init() {
        ...
        this.setupEscapeKey();
        console.log('WindowManager initialized with', Object.keys(this.registry).length, 'windows');
    }
