...
class InputSanitizer {
   ...
   */
  escapeHTML(value) {
    return String(value).replace(/[&<>"'`=\/]/g, char => this.htmlEntities[char]);
  }
   ...
   */
  unescapeHTML(value) {
    const reverseEntities = Object.entries(this.htmlEntities).reduce((acc, [char, entity]) => {
      acc[entity] = char;
      return acc;
    }, {});
    
    return String(value).replace(/&[#\w]+;/g, entity => reverseEntities[entity] || entity);
  }
  ...
  sanitizeFileName(value) {
    ...
    if (value.length > 255) {
      ...
      const nameWithoutExt = value.substring(0, value.length - extension.length);
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (schema && schema[sanitizedKey]) {
        if (typeof schema[sanitizedKey] === 'string') {
          ...
          sanitized[sanitizedKey] = this.createSanitizedCopy(value, schema[sanitizedKey]);
        } else {
