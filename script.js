...
class OTTOAccurateInterface {
  constructor() {
    ...
    this.tapTimes = [];
    this.maxTapTimes = 4;
    this.isPlaying = false; // Start in pause state to show play icon
  ...
  setupSettingsWindow() {
    ...
    if (settingsSystemResetBtn) {
      settingsSystemResetBtn.addEventListener("click", async () => {
  ...
  async init() {
    try {
      ...
      this.populateKitDropdown(); // Re-populate after controls are setup
  ...
  setupPlayerTempoControls() {
    // COMMENTED OUT - Clean up existing tempo button listeners
    /* if (this.eventListenerRegistry && this.eventListenerRegistry.element) {
      this.eventListenerRegistry.element = this.eventListenerRegistry.element.filter(
        ({ element, event, handler }) => {
          if (
    ...
    // COMMENTED OUT - Get all tempo control buttons
    ...
    // COMMENTED OUT - Setup double tempo buttons
    ...
    // COMMENTED OUT - Setup half tempo buttons
    ...
    // COMMENTED OUT - Update tempo button states for all players
    // this.updateAllPlayerTempoStates();
  }

  /**
   * COMMENTED OUT - Handle double tempo for a player
  ...
  /**
   * COMMENTED OUT - Handle half tempo for a player
   * @param {number} playerNum - Player number (1-8)
   */
  ...
  updatePlayerTempoState(playerNum) {
    ...
    // COMMENTED OUT - Update button states
  ...
  /**
   * COMMENTED OUT - Update tempo states for all players
   */
  ...
  populateKitDropdown() {
    ...
    if (!kitOptionsContainer) {
      debugError("Kit options container not found");
      return;
    }
  ...
  setupPatternGroupControls() {
    ...
    if (groupPrev) {
      ...
      this.addEventListener(groupPrev, "click", prevHandler, "dropdown");
    }
    ...
    if (groupNext) {
      ...
      this.addEventListener(groupNext, "click", nextHandler, "dropdown");
    }
    ...
    if (editPatternBtn) {
      ...
      this.addEventListener(editPatternBtn, "click", editHandler, "dropdown");
    }
    ...
    if (panelCloseBtn) {
      ...
      this.addEventListener(panelCloseBtn, "click", closeHandler, "dropdown");
    }
    ...
    if (deleteBtn) {
      ...
      this.addEventListener(deleteBtn, "click", deleteHandler, "dropdown");
    }
    ...
    if (searchInput) {
      ...
      this.addEventListener(searchInput, "input", searchHandler, "dropdown");
    }
  ...
  setupPatternGrid() {
    ...
    patternButtons.forEach((patternBtn) => {
      ...
      // Use enhanced event listener management
      this.addEventListener(patternBtn, "click", clickHandler, "dropdown");
    });
  }
  ...
  setupToggleButtons() {
    ...
    document.querySelectorAll(".toggle-btn").forEach((toggleBtn) => {
      ...
      // Use enhanced event listener management
      this.addEventListener(toggleBtn, "click", clickHandler, "dropdown");
    });
  }
  ...
  setupFillButtons() {
    ...
    document.querySelectorAll(".fill-btn").forEach((fillBtn) => {
      ...
      // Use enhanced event listener management
      this.addEventListener(fillBtn, "click", clickHandler, "dropdown");
    });
  }
