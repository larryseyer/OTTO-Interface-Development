...
class DataValidator {
  ...
  initializeValidators() {
    ...
    this.addValidator('object', value => value !== null && typeof value === 'object' && !Array.isArray(value));
    ...
    this.addValidator('negative', value => typeof value === 'number' && value < 0);
    this.addValidator('nonNegative', value => typeof value === 'number' && value >= 0);
    ...
    // Array validators
    this.addValidator('nonEmptyArray', value => Array.isArray(value) && value.length > 0);
    ...
    this.addValidator('patternName', value => {
      ...
      return typeof value === 'string' && value.length > 0 && value.length <= 50 && regex.test(value);
    });
  ...
  initializeSchemas() {
    ...
    this.addSchema('playerState', {
      ...
      selectedPattern: { type: 'string', required: true, validator: 'patternName' },
      ...
      fillStates: {
        ...
        custom: (value) => {
          ...
          const exclusiveFills = [value['4'], value['8'], value['16'], value['32']];
    ...
    this.addSchema('globalState', {
      ...
      currentPlayer: { type: 'number', required: true, validator: 'playerNumber' },
      numberOfPlayers: { 
        type: 'number', 
        required: true, 
        min: 1, 
  ...
  validateProperty(value, schema, path) {
    ...
    if (typeof value === 'string' || Array.isArray(value)) {
      if (schema.minLength !== undefined && value.length < schema.minLength) {
        this.addError(`${path} must have at least ${schema.minLength} items/characters`);
        return false;
      }
      if (schema.maxLength !== undefined && value.length > schema.maxLength) {
        this.addError(`${path} must have at most ${schema.maxLength} items/characters`);
        return false;
      }
      if (schema.length !== undefined && value.length !== schema.length) {
        this.addError(`${path} must have exactly ${schema.length} items/characters`);
        return false;
      }
    }
  ...
  validateObjectProperties(value, schema, path) {
    ...
    if (schema.minProperties !== undefined && propCount < schema.minProperties) {
      this.addError(`${path} must have at least ${schema.minProperties} properties`);
      valid = false;
    }
    if (schema.maxProperties !== undefined && propCount > schema.maxProperties) {
      this.addError(`${path} must have at most ${schema.maxProperties} properties`);
      valid = false;
    }
    ...
    if (schema.additionalProperties === false) {
      ...
      for (const [key, val] of Object.entries(value)) {
        if (!definedKeys.has(key)) {
          const propPath = path ? `${path}.${key}` : key;
          if (!this.validateProperty(val, schema.additionalProperties, propPath)) {
            valid = false;
          }
        }
      }
  ...
  getStats() {
    return {
      ...
      cacheHitRate: this.stats.validations > 0 
