...
class PatternGroupManager {
  constructor(storageManager) {
    ...
    this.availablePatterns = [
      ...
      'rock-steady', 'reggae', 'hip-hop', 'trap',
      'dubstep', 'house', 'techno', 'trance',
      'ambient', 'chill', 'lofi', 'boom-bap',
      'breaks', 'dnb', 'garage', 'grime'
    ];
  ...
  createGroup(name, patterns = null) {
    ...
    if (!this.validatePatternArray(groupPatterns)) {
      throw new Error('Invalid pattern array - must contain exactly 16 patterns');
    }
  ...
  updateGroup(key, updates) {
    ...
    if (updates.patterns && !updates.patterns.includes(updatedGroup.selectedPattern)) {
  ...
  swapPatterns(groupKey, position1, position2) {
    if (position1 < 0 || position1 >= this.PATTERNS_PER_GROUP ||
        position2 < 0 || position2 >= this.PATTERNS_PER_GROUP) {
    ...
    [newPatterns[position2], newPatterns[position1]];
  ...
  renameGroup(groupKey, newName) {
    ...
    if (newKey !== groupKey) {
      ...
      // Save and notify
      this.saveToStorage();
      this.notifyListeners('rename', newKey, updatedGroup, { ...group, key: groupKey });
