class MixerPresetManager {
  ...
  setCurrentGlobalPreset(id) {
    ...
    this.saveToStorage();
    this.notifyListeners('global-preset-selected', { id, preset: this.globalPresets.get(id) });
  ...
  createKitPreset(kitKey, presetName, basePresetData = null) {
    ...
    if (typeof basePresetData === 'string' && this.globalPresets.has(basePresetData)) {
    ...
    this.useGlobalPresets = false;
    return this.drumkitManager.createMixerPreset(kitKey, presetName, basePresetData);
  }
  ...
  applyGlobalPresetToKit(globalPresetId, kitKey) {
    ...
    this.useGlobalPresets = false;
    return this.drumkitManager.setMixerPreset(kitKey, presetName, kitPresetData);
  }
  ...
  updateChannel(channel, updates, kitKey = null) {
    if (this.useGlobalPresets) {
      ...
      if (preset.isFactory) {
        ...
        const newId = this.createGlobalPreset(preset.name + ' (Modified)', preset.id);
      ...
      this.drumkitManager.setMixerPreset(kitKey, preset.name || 'default', preset);
      this.notifyListeners('channel-updated', { channel, updates, preset, kitKey });
  ...
  importPreset(presetData, asGlobal = true) {
    ...
    if (asGlobal) {
      ...
      throw new Error('Kit preset import should be done through DrumkitManager');
    }
  }
  ...
  getStats() {
    ...
    return {
      ...
      currentGlobalPreset: this.currentGlobalPreset,
      useGlobalPresets: this.useGlobalPresets,
      kitPresetStats: this.drumkitManager ? this.drumkitManager.getStats() : null
    };
  }
