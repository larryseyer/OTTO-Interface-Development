...
class AudioScheduler {
  ...
  scheduleNote(beatNumber, time) {
    ...
    this.notesInQueue = this.notesInQueue.filter(n => n.time > currentTime - 0.1);
  ...
  setTempo(bpm) {
    ...
    if (this.isPlaying) {
      ...
      const timeSinceLastNote = this.audioContext.currentTime - 
        (this.nextNoteTime - (60.0 / this.tempo / 4));
      this.nextNoteTime = this.audioContext.currentTime + 
        (timeSinceLastNote * ratio);
    }
  ...
  syncToClock(masterClock) {
    ...
    const offset = masterClock.audioContext.currentTime - this.audioContext.currentTime;
