...
class PresetManager {
  ...
  captureCurrentState() {
    return {
      players: this.playerStateManager
        ? this.playerStateManager.exportStates()
        : {},
      patternGroups: {
        current: this.patternGroupManager
          ? this.patternGroupManager.currentGroupKey
          : "favorites",
      ...
      drumkits: {
        ...
        playerKits: this.drumkitManager
          ? Array.from(this.drumkitManager.playerKits.entries()).reduce(
              (acc, [player, kit]) => {
                acc[player] = kit;
                return acc;
              },
  ...
  savePreset(key = null) {
    ...
    if (presetKey === "default") {
      ...
      console.warn(
        "Saving to default preset - this will affect all new presets",
      );
    }
  ...
  applyPresetData(data) {
    ...
    if (data.patternGroups && this.patternGroupManager) {
      if (
        data.patternGroups.current &&
        this.patternGroupManager.groups.has(data.patternGroups.current)
      ) {
  ...
  validatePresetName(name) {
    ...
    if (name.length === 0 || name.length > this.MAX_PRESET_NAME_LENGTH)
      return false;
  ...
  async loadFromStorage() {
    ...
    try {
      ...
      if (
        data &&
        data.currentPresetKey &&
        this.presets.has(data.currentPresetKey)
      ) {
